...
//  To Do List
//
//  Created by Matt Giovanniello on 9/25/17.
...
//    var toDoNotesArray = ["I should be certain to do all of the exercises at the end of the chapters before the exam.", "take my ideas to the school's venture competition and win the big check.", "Focus apps on empowerment for all, with an extra bonus for users who are kind."]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "EditItem" {   // did we click on cell?
            let destination = segue.destination as! DetailViewController   // force convert to Detail View Controller
            let index = tableView.indexPathForSelectedRow!.row   // can force unwrap because it'll never be nil
